...
class es_webform_magento_product_attributeTest extends PHPUnit_Framework_TestCase
{
  ...
  public function testattribute_exists() {
	...
	public function testcreateProductAddData() {
		...
		var_export((object)$result);
		var_export($this->getProductCreateDataSample());
		$sample = $this->getProductCreateDataSample();
		$this->assertEquals(count($sample['attributes']), count($result['attributes'])); //this is the best i can do asserting equals on the arrays is not working out b/c of 
